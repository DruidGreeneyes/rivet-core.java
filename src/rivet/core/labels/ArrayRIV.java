...
public class ArrayRIV implements RIV, Serializable {
	...
	@Override
	public ArrayRIV destructiveAdd(final RIV other) throws SizeMismatchException {
		if (size == other.size()) {
			other.keyStream().forEach((k) -> destructiveSet(getPoint(k).add(other.get(k))));
			return this;
		} else
			throw new SizeMismatchException("Target RIV is the wrong size!");
	}

	private void destructiveSet(final VectorElement elt) throws IndexOutOfBoundsException {
		if (validIndex(elt.index())) {
			...
			throw new IndexOutOfBoundsException("Index " + elt.index() + " is outside the bounds of this vector.");
	}

	@Override
	public ArrayRIV destructiveSub(final RIV other) throws SizeMismatchException {
		if (size == other.size()) {
			other.keyStream().forEach((k) -> destructiveSet(getPoint(k).subtract(other.get(k))));
			return this;
		} else
			throw new SizeMismatchException("Target RIV is the wrong size!");
	}
	...
	protected ArrayRIV mapVals(final DoubleUnaryOperator fun) {
		return new ArrayRIV(keyStream().toArray(), valStream().map(fun).toArray(), size).removeZeros();
	}
	...
	private ArrayRIV removeZeros() {
		final VectorElement[] elts = stream().filter(ve -> !ve.contains(0)).toArray(VectorElement[]::new);
	...
	public String toString() {
		// "0|1 1|3 4|2 5"
		// "I|V I|V I|V Size"
		return stream().map(VectorElement::toString).collect(Collectors.joining(" ", "", " " + String.valueOf(size)));
	}

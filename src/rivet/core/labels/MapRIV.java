...
public class MapRIV extends ConcurrentHashMap<Integer, Double> implements RIV {
	...
	public MapRIV(final ConcurrentHashMap<Integer, Double> points, final int size) {
		super(points);
		this.size = size;
	}
	...
	private void assertSizeMatch(final RIV other, final String message) throws SizeMismatchException {
		if (size != other.size())
	 ...
	 * @throws SizeMismatchException
	 */
	public MapRIV destructiveAdd(final MapRIV other) throws SizeMismatchException {
		assertSizeMatch(other, "Cannot add rivs of mismatched sizes.");
	...
	public MapRIV permute(final Permutations permutations, final int times) {
		if (times == 0)
			...
			return new MapRIV(times > 0 ? permuteKeys(keyStream(), permutations.left, times)
					: permuteKeys(keyStream(), permutations.right, -times), valStream().toArray(), size);
	}

	@Override
	public VectorElement[] points() {
		return stream().map(VectorElement::elt).sorted(VectorElement::compare).toArray(VectorElement[]::new);
	}
	...
	public MapRIV removeZeros() {
		final ConcurrentHashMap<Integer, Double> map = entrySet().stream()
				...
				.collect(ConcurrentHashMap::new, (i, e) -> i.put(e.getKey(), e.getValue()), ConcurrentHashMap::putAll);
	...
	public String toString() {
		// "0|1 1|3 4|2 5"
		// "I|V I|V I|V Size"
		return stream().sorted((e1, e2) -> Integer.compare(e1.getKey(), e2.getKey()))
